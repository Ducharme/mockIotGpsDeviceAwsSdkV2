...
export STREAMING_LOCATION_TOPIC=$(grep STREAMING_LOCATION_TOPIC $ENV_FILE | cut -d '=' -f2)
export STREAMID_REQUEST_TOPIC=$(grep STREAMID_REQUEST_TOPIC $ENV_FILE | cut -d '=' -f2)
export STREAMID_REPLY_TOPIC=$(grep STREAMID_REPLY_TOPIC $ENV_FILE | cut -d '=' -f2)
...
echo dist/main.js ENDPOINT=$ENDPOINT STREAMING_LOCATION_TOPIC=$STREAMING_LOCATION_TOPIC --streamIdRequestTopic $STREAMID_REQUEST_TOPIC --streamIdReplyTopic $STREAMID_REPLY_TOPIC COUNT=$COUNT INTERVAL=$INTERVAL CA_FILE=$CA_FILE CERT_FILE=$CERT_FILE KEY_FILE=$KEY_FILE
node dist/main.js
